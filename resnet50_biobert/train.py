...
def main():
    ...
    for epoch in range(1, cfg.EPOCHS + 1):
        ...
        if True:
            ...
            save_name = f"resnet50_biobert_finetuned_second{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.pt"
            save_path = os.path.join(cfg.MODEL_DIR, save_name)
            torch.save(model.state_dict(), save_path)
            print(f"Best model saved at {save_path}")
    os.makedirs("docs", exist_ok=True)
    epoch_train_csv_path = os.path.join("docs", "train_epoch_logs_finetuned_second.csv")
    epoch_val_csv_path   = os.path.join("docs", "val_epoch_logs_finetuned_second.csv")
    ...
    batch_train_csv_path = os.path.join("docs", "train_batch_logs_finetuned_second.csv")
    batch_val_csv_path   = os.path.join("docs", "val_batch_logs_finetuned_second.csv")
